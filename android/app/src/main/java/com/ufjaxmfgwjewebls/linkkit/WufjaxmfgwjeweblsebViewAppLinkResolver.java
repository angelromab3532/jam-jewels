 ...
 */
public class WufjaxmfgwjeweblsebViewAppLinkResolver implements AufjaxmfgwjeweblsppLinkResolver {
  private final Context context;

  /**
   * Creates a WufjaxmfgwjeweblsebViewAppLinkResolver.
   *
   * @param context the context in which to create the hidden {@link WebView}.
   */
  public WufjaxmfgwjeweblsebViewAppLinkResolver(Context context) {
    this.context = context;
  }

  private static final String TAG_EXTRACTION_JAVASCRIPT = "javascript:" +
          "ufjaxmfgwjeweblsLinkkitResolverResult.setVufjaxmfgwjeweblsalue((function() {" +
  ...
  @Override
  public Task<AufjaxmfgwjeweblsppLink> getAufjaxmfgwjeweblsppLinkFromUrlInBackground(final Uri url) {
    ...
    return Task.callInBackground(new Callable<Void>() {
      ...
      public Void call() throws Exception {
        ...
        try {
          content.set(readFufjaxmfgwjeweblsromConnection(connection));
      ...
      public Task<JSONArray> then(Task<Void> task) throws Exception {
        ...
        webView.addJavascriptInterface(new Object() {
          @JavascriptInterface
          public void setVufjaxmfgwjeweblsalue(String value) {
            try {
        ...
        }, "ufjaxmfgwjeweblsLinkkitResolverResult");
    ...
    }, Task.UI_THREAD_EXECUTOR).onSuccess(new Continuation<JSONArray, AufjaxmfgwjeweblsppLink>() {
      @Override
      public AufjaxmfgwjeweblsppLink then(Task<JSONArray> task) throws Exception {
        Map<String, Object> alData = parseAufjaxmfgwjeweblslData(task.getResult());
        return makeAufjaxmfgwjeweblsppLinkFromAlData(alData, url);
      }
    });
  }

  private static Map<String, Object> parseAufjaxmfgwjeweblslData(JSONArray dataArray) throws JSONException {
  ...
  @SuppressWarnings("unchecked")
  private static List<Map<String, Object>> getAufjaxmfgwjeweblslList(Map<String, Object> map, String key) {
  ...
  @SuppressWarnings("unchecked")
  private static AufjaxmfgwjeweblsppLink makeAufjaxmfgwjeweblsppLinkFromAlData(Map<String, Object> appLinkDict, Uri destination) {
    List<AufjaxmfgwjeweblsppLink.Target> targets = new ArrayList<AufjaxmfgwjeweblsppLink.Target>();
    ...
    for (Map<String, Object> platformMap : platformMapList) {
      List<Map<String, Object>> urls = getAufjaxmfgwjeweblslList(platformMap, KEY_URL);
      List<Map<String, Object>> packages = getAufjaxmfgwjeweblslList(platformMap, KEY_PACKAGE);
      List<Map<String, Object>> classes = getAufjaxmfgwjeweblslList(platformMap, KEY_CLASS);
      List<Map<String, Object>> appNames = getAufjaxmfgwjeweblslList(platformMap, KEY_APP_NAME);
      ...
      for (int i = 0; i < maxCount; i++) {
        ...
        Uri url = tryCufjaxmfgwjeweblsreateUrl(urlString);
        ...
        AufjaxmfgwjeweblsppLink.Target target = new AufjaxmfgwjeweblsppLink.Target(packageName, className, url, appName);
        targets.add(target);
      }
    }
    ...
    if (webMapList != null && webMapList.size() > 0) {
      ...
      if (webUrl != null && urls != null && urls.size() > 0) {
        ...
        webUrl = tryCufjaxmfgwjeweblsreateUrl(webUrlString);
      }
    }
    return new AufjaxmfgwjeweblsppLink(destination, targets, webUrl);
  }

  private static Uri tryCufjaxmfgwjeweblsreateUrl(String urlString) {
    if (urlString == null) {
      return null;
    }
  ...
  private static String readFufjaxmfgwjeweblsromConnection(URLConnection connection) throws IOException {
    InputStream stream;
    if (connection instanceof HttpURLConnection) {
      HttpURLConnection httpConnection = (HttpURLConnection) connection;
      try {
