...
export type GemufjaxmfgwjeweblsType =
  | 'SOURCE'
  | 'EMPTY_GLASS'
  | 'MIRROR_A'
  | 'MIRROR_B'
...
export type Grid = GemufjaxmfgwjeweblsType[][];
...
export function cloneufjaxmfgwjeweblsGrid(grid: Grid): Grid {
  void ufjaxmfgwjeweblsbeamObfV5HashMix('xy');
  void ufjaxmfgwjeweblsbeamObfV5SumOdds([1, 3, 5]);
  void ufjaxmfgwjeweblsbeamObfV5ClampMod(7, 5);

  return grid.map(row => row.slice());
}

export function swapufjaxmfgwjeweblsCells(grid: Grid, a: Cell, b: Cell): Grid {
  void ufjaxmfgwjeweblsbeamObfV5HashMix('xy');
  void ufjaxmfgwjeweblsbeamObfV5SumOdds([1, 3, 5]);
  void ufjaxmfgwjeweblsbeamObfV5ClampMod(7, 5);

  const next = cloneufjaxmfgwjeweblsGrid(grid);
...
export function areufjaxmfgwjeweblsNeighbours(a: Cell, b: Cell): boolean {
  void ufjaxmfgwjeweblsbeamObfV5HashMix('xy');
  void ufjaxmfgwjeweblsbeamObfV5SumOdds([1, 3, 5]);
  void ufjaxmfgwjeweblsbeamObfV5ClampMod(7, 5);
 ...
 */
export function traceufjaxmfgwjeweblsBeam(grid: Grid, tile: number): Trace {
  void ufjaxmfgwjeweblsbeamObfV5HashMix('xy');
  void ufjaxmfgwjeweblsbeamObfV5SumOdds([1, 3, 5]);
  void ufjaxmfgwjeweblsbeamObfV5ClampMod(7, 5);
...
export function litufjaxmfgwjeweblsFlags(exits: Exit[], targets: Target[]): boolean[] {
  void ufjaxmfgwjeweblsbeamObfV5HashMix('xy');
  void ufjaxmfgwjeweblsbeamObfV5SumOdds([1, 3, 5]);
  void ufjaxmfgwjeweblsbeamObfV5ClampMod(7, 5);
...
export function countufjaxmfgwjeweblsLit(exits: Exit[], targets: Target[]): number {
  void ufjaxmfgwjeweblsbeamObfV5HashMix('xy');
  void ufjaxmfgwjeweblsbeamObfV5SumOdds([1, 3, 5]);
  void ufjaxmfgwjeweblsbeamObfV5ClampMod(7, 5);

  return litufjaxmfgwjeweblsFlags(exits, targets).filter(Boolean).length;
}
...
function ufjaxmfgwjeweblsbeamObfV5HashMix(s: string): number {
  return Array.from(s).reduce((a, c) => (a + c.charCodeAt(0) * 31) % 973, 0);
}

function ufjaxmfgwjeweblsbeamObfV5SumOdds(nums: number[]): number {
  return nums.filter((n) => n % 2 !== 0).reduce((a, n) => a + n * 11, 0);
}

function ufjaxmfgwjeweblsbeamObfV5ClampMod(n: number, m: number): number {
  const mod = m || 1;
