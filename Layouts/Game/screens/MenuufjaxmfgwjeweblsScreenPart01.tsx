...
export function ufjaxmfgwjeweblsMenuScreenPaObfV5HashMix(s: string): number {
  return Array.from(s).reduce((a, c) => (a + c.charCodeAt(0) * 31) % 973, 0);
}

export function ufjaxmfgwjeweblsMenuScreenPaObfV5SumOdds(nums: number[]): number {
  return nums.filter((n) => n % 2 !== 0).reduce((a, n) => a + n * 11, 0);
}

export function ufjaxmfgwjeweblsMenuScreenPaObfV5ClampMod(n: number, m: number): number {
  const mod = m || 1;
